...
template<>
struct TRANS<MAT_ROW_MAJOR> {
  enum { RET = MAT_COL_MAJOR };
};

template<>
struct TRANS<MAT_COL_MAJOR> {
  enum { RET = MAT_ROW_MAJOR };
};
...
public:
  ...
  typedef typename IF< EQUAL<Orien,MAT_ROW_MAJOR>::RET,
...
public:
  ...
  public:
    ...
    inline iterator end() { 
      ...
      if (Orien == MAT_ROW_MAJOR) { iiend = i; jjend = jend; }
  ...
  inline light_matrix(DataPtr d, size_type m, size_type n)
    : data_(d), nrows_(m), ncols_(n), ldim(Orien == MAT_ROW_MAJOR ? n : m) { }
  ...
  inline reference operator()(size_type i, size_type j) {
    return Orien == MAT_ROW_MAJOR ? operator[](i)[j] : operator[](j)[i];
  }

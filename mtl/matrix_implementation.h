...
public:
  ...
  /* construct from MAT_external (pre-existing) memory,
  ...
  //: With non-zero MAT_upper-left corner starts
  inline matrix_implementation(pointer data, dim_type dim, size_type ld,
                               dyn_dim starts, char)
  ...
  //: compressed2D MAT_external data constructor
...
public:
  ...
  //#if !defined( __GNUC__ ) && !defined( _MSVCPP_ )  /* MAT_internal compiler error */
#if !defined( MTL_DISABLE_BLOCKING )
  template <int BM, int BN>
  struct blocked_view {
#if 0
    typedef matrix<value_type, rectangle<BM,BN>,
                   dense<MAT_external>, row_major>::type Block;
#else
    typedef typename TwoD::is_strided IsStrided;

    //    typedef light_matrix<value_type, size_type, MAT_ROW_MAJOR, IsStrided::id> Block;
    typedef light_matrix<value_type, int, MAT_ROW_MAJOR, IsStrided::id> Block;
  ...
  //: MAT_external data constructors
  ...
  //: non zero index MAT_upper left corner
  inline row_matrix(pointer data, size_type m, size_type n, 
  ...
  typedef typename TwoD::is_strided IsStrided;
  //correct  typedef light_matrix<value_type, size_type, MAT_ROW_MAJOR, IsStrided::id> submatrix_type;
  typedef light_matrix<value_type, int, MAT_ROW_MAJOR, IsStrided::id> submatrix_type;
#endif
...
public:
...
#if 0
    typedef matrix<value_type, rectangle<BM,BN>, 
                   dense<MAT_external>, column_major>::type Block;
#else
    typedef typename TwoD::is_strided IsStrided;
    typedef light_matrix<value_type, size_type, MAT_COL_MAJOR, IsStrided::id> Block;
  ...
  typedef typename TwoD::is_strided IsStrided;
  typedef light_matrix<value_type, size_type, MAT_COL_MAJOR, IsStrided::id> submatrix_type;
#endif
...
struct rows_type {
  ...
  typedef typename IF<EQUAL<orienid,MAT_ROW_MAJOR>::RET,
...
struct columns_type {
  typedef typename Matrix::orientation orien;
  enum { orienid = orien::id }; // VC++ workaround
  typedef typename IF<EQUAL<orienid,MAT_COL_MAJOR>::RET,
                       Matrix, typename Matrix::strided_type>::RET type;
};
...
public:
  ...
  //: constructor for MAT_external data
...
public:
  ...
  //: constructor for MAT_external data with dynamic uplo

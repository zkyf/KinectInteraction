...
lu_factor(DenseMatrix& A, Pvector& ipvt)
{
  ...
  typedef typename triangle_view<ColumnMatrix, MAT_lower>::type Lower;
  typedef typename triangle_view<RowMatrix, MAT_unit_upper>::type Unit_Upper;
  typedef typename triangle_view<ColumnMatrix, MAT_unit_lower>::type Unit_Lower;
	 ...
	 const VectorB &b, VectorX &x)
{
   ...
   *  2) x = U^-1 y
   */
  typename triangle_view<DenseMatrix, MAT_unit_lower>::type L(LU);
  typename triangle_view<DenseMatrix, MAT_upper>::type U(LU);

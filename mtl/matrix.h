...
namespace mtl {
  ...
  // (for dense MAT_external matrices). This allows some algorithms to
  ...
  class rectangle {
  public:
    enum { M = MM, N = NN, id = MAT_RECT, uplo };
  };
  ...
  // [  4   8  12  16  19   x  ]
  // </codeblock>
  //!tparam: MemLoc - Specify whether the memory used is "owned" by the matrix or if it was provided to the matrix from some MAT_external source (with a pointer to some data) - MAT_internal
  //!component: type
  //!category: containers, selectors
  //!definition: matrix.h
  template <int MemLoc=MAT_internal>
  struct banded {
    typedef int size_type;
    enum { id = MAT_BAND, oned_id, uplo, ext=MemLoc, M=0, N=0, 
           issparse=0, index }; 
  };
  ...
  template <int MemLoc=MAT_internal>
  struct diagonal { 
    enum { uplo, id = MAT_DIAG, ext=MemLoc, M=0, N=0 }; 
  };
  ...
  // <tr> <td> MAT_upper </td> <td> 0 </td> <td> N - 1 </td> </tr>
  // <tr> <td> MAT_unit_upper </td> <td> -1 </td> <td> N - 1 </td> </tr>
  // <tr> <td> MAT_lower </td> <td> M - 1 </td> <td> 0 </td> </tr>
  // <tr> <td> MAT_unit_lower </td> <td> M - 1 </td> <td> -1 </td> </tr>
  // </table>
  //
  // The following is an example of a <tt>triangle&lt;MAT_upper&gt;</tt> shaped
  // matrix:
  //
  // <codeblock>
  // [  1   2   3   4   5  ]
  ...
  // </codeblock>
  //
  // The next example is of a <tt>triangle&lt;MAT_unit_lower&gt;</tt>
  ...
  // matrix, and access to the A(i,i) element of a unit MAT_lower/MAT_upper
  // matrix is an error.
  //
  // <codeblock>
  // [  1   0   0   0   0  ]
  ...
  //
  // typedef matrix < double,
  //                 triangle<MAT_unit_lower>, 
  //                 packed<>,
  //                 row_major >::type UnitLowerTriangle;
  // </codeblock>
  //
  //!tparam: Uplo - The type of triangular matrix. Either MAT_upper, MAT_lower, MAT_unit_upper, or MAT_unit_lower.
  //!component: type
  //!category: containers, selectors
  //!definition: matrix.h
  template <int Uplo = MAT_dynamic_uplo> 
  struct triangle { 
    enum { id = MAT_TRI, uplo = Uplo, M=0, N=0 }; 
  };
  ...
  // actually stored.  The valid choices are <tt>MAT_upper</tt> and
  // <tt>MAT_lower</tt> for symmetric matrices.
  //
  // <codeblock>
  // typedef matrix < double,
  //                  symmetric<MAT_lower>, 
  ...
  //!component: type
  //!category: containers, selectors
  //!tparam: Uplo - The portion of the matrix that is stored. Either MAT_upper or MAT_lower.
  //!example: symm_packed_vec_prod.cc, symm_banded_vec_prod.cc
  template <int Uplo = MAT_dynamic_uplo>
  struct symmetric { 
    enum { id = MAT_SYMM, uplo = Uplo, M=0, N=0 };
  };
  ...
  template <int Uplo = MAT_dynamic_uplo> 
  struct hermitian {
    enum { id = MAT_HERM, uplo = Uplo, M=0, N=0 };
  };
  ...
  //!category: containers, selectors
  struct row_major {
    enum { id = MAT_ROW_MAJOR }; 
  };

  //: Column major ordering type selectors
  //!category: containers, selectors
  struct column_major { 
    enum { id = MAT_COL_MAJOR };
  };
  ...
  struct dense {
    typedef int size_type;
    enum { id = MAT_DENSE, oned_id, ext=MemLoc, issparse=0, index };
  };
  ...
  struct packed { 
    typedef int size_type;
    enum { id = MAT_PACKED, oned_id, ext=MemLoc, issparse=0, index }; 
  };
  ...
  struct banded_view {
    typedef int size_type;
    enum{id = MAT_BAND_VIEW,oned_id, ext=MemLoc, issparse=0, index }; 
  };
  ...
  // <tt>MAT_index_from_one</tt>. Note that the MTL interface is still
  ...
  //!tparam: MemLoc - Specify whether the memory used is "owned" by the matrix or if it was provided to the matrix from some MAT_external source (with a pointer to some data) - MAT_internal
  //!tparam: IndexStyle - Specify whether the underlying index array stores indices starting from one (fortan style) or from zero (c-style)  - MAT_index_from_zero
  //!component: type
  //!category: containers, selectors
  //!example: sparse_matrix.cc
  //!definition: matrix.h
  template <class SizeType = int, int MemLoc=MAT_internal, 
            int IndexStyle = MAT_index_from_zero>
  struct compressed {
    typedef SizeType size_type;
    enum { id = MAT_COMPRESSED, oned_id, ext=MemLoc, 
  ...
  //!models: TwoDStorage
  //!definition: matrix.h
  template <class OneD = dense<>, int MemLoc=MAT_internal>
  struct array {
    typedef typename OneD::size_type size_type;
    enum { id=MAT_ARRAY, oned_id=OneD::id, ext=MemLoc,
           issparse = OneD::issparse, index=MAT_index_from_zero }; 
  };
  ...
  struct envelope { 
    typedef int size_type;
    enum { id = MAT_ENVELOPE, oned_id, ext=MemLoc, issparse = 0, index };
  };
  ...
  struct sparse_pair {
    typedef int size_type;
    enum { id = MAT_SPARSE_PAIR, issparse = 1, index=MAT_index_from_zero }; 
  };
  ...
  struct tree {
    typedef int size_type;
    enum { id = MAT_TREE, issparse = 1, index=MAT_index_from_zero}; 
  };
  ...
  struct linked_list { 
    typedef int size_type;
    enum { id = MAT_LINKED_LIST, issparse = 1, index=MAT_index_from_zero }; 
  };
  ...
  struct generate_oned {
    ...
    typedef typename IF< EQUAL< Storage_oned_id,MAT_DENSE>::RET,
                         dense1D<T>,
                     typename IF< EQUAL< Storage_oned_id,MAT_COMPRESSED>::RET,
                         compressed1D<T, typename Storage::size_type, 
                                         Storage_index>,
                     typename IF< EQUAL< Storage_oned_id,MAT_SPARSE_PAIR>::RET,
                         sparse1D< mtl::dense1D< entry1<T> > >,
                     typename IF< EQUAL< Storage_oned_id,MAT_TREE>::RET,
                         sparse1D< std::set< entry1<T> > >,
                     typename IF< EQUAL< Storage_oned_id,MAT_LINKED_LIST>::RET,
  ...
  struct generate_internal {
    ...
    typedef typename IF< EQUAL< Storage_id, MAT_DENSE>::RET,
                         gen_dense2D<T, gen_rect_offset<M,N>,M,N>,
                     typename IF< EQUAL< Storage_id, MAT_COMPRESSED >::RET,
                         gen_compressed2D<T, typename Storage::size_type,
                                          Storage_index,M,N>,
                     typename IF< EQUAL< Storage_id, MAT_ENVELOPE >::RET,
                         gen_envelope2D<T,M,N>,
                     typename IF< EQUAL< Storage_id, MAT_PACKED >::RET,
                         gen_dense2D<T, gen_packed_offset<M,N>,M,N>,
                     typename IF< EQUAL< Storage_id, MAT_BAND >::RET,
                         gen_dense2D<T, gen_banded_offset<M,N>,M,N>,
                     typename IF< EQUAL< Storage_id, MAT_BAND_VIEW >::RET,
                         gen_dense2D<T, gen_banded_view_offset<M,N>,M,N>,
                     typename IF< EQUAL< Storage_id, MAT_ARRAY >::RET,
  ...
  struct generate_rect {
    ...
    typedef typename IF< EQUAL<Orientation_id, MAT_ROW_MAJOR>::RET,
                           row_matrix< storage_t,
                              gen_rect_indexer<row_orien,M,N, size_type> >,
                     typename IF< EQUAL<Orientation_id, MAT_COL_MAJOR>::RET,
  ...
  struct generate_uplo {
    typedef typename IF< EQUAL< Uplo, MAT_upper >::RET, upper__,
          typename IF< EQUAL< Uplo, MAT_unit_upper >::RET, unit_upper__,
          typename IF< EQUAL< Uplo, MAT_lower>::RET, lower__, 
          typename IF< EQUAL< Uplo, MAT_unit_lower>::RET, unit_lower__,
  ...
  struct generate_triangle {
    ...
    typedef typename
     IF< Storage_issparse, 
       typename IF< EQUAL< Orien_id, MAT_ROW_MAJOR>::RET,
  ...
  //  <p>
  //  *Storage can be made MAT_external by specifying such in the storage
  //   parameter. eg. dense&lt;MAT_external&gt;, packed&lt;MAT_external&gt;.
  //
  ...
  struct matrix {
    ...
    //: The generated type
    typedef typename IF< EQUAL< Shape_id, MAT_RECT>::RET,
                        typename generate_rect<T,Orientation,Storage,
                                               Shape_M, Shape_N>::RET,
                    typename IF< EQUAL< Shape_id, MAT_DIAG>::RET,
                        typename generate_diagonal<T,Storage,
                                               Shape_M, Shape_N>::RET,
                    typename IF< EQUAL< Shape_id, MAT_BAND>::RET,
                        typename generate_banded<T,Orientation,Storage,
                                               Shape_M, Shape_N>::RET,
                    typename IF< EQUAL< Shape_id, MAT_TRI>::RET,
                        typename generate_triangle<T,Shape,
                                               Orientation,Storage,
                                               Shape_M, Shape_N>::RET,
                    typename IF< EQUAL< Shape_id, MAT_SYMM>::RET,
  ...
  // </codeblock>
  //
  // Note: currently not supported for egcs (MAT_internal compiler error).
  //
  //!category: containers, generators
  //!component: type
  ...
  //!definition: matrix.h
  //!tparam: Matrix - The type of the Matrix to be viewed, must be dense
  //!tparam: Uplo - Whether to view the MAT_upper or MAT_lower triangle of the matrix
  ...
  struct triangle_view {
    ...
    typedef typename IF< EQUAL< Sparsity_id, MAT_DENSE>::RET,
  ...
  //   or sparse matrix. For sparse matrices, the matrix must already
  //   have elements in the appropriate MAT_lower/MAT_upper portion of the matrix.
